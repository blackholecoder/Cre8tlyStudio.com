...
export default function BookGrid({ books = [], onAddPrompt, onGenerateNext }) {
  ...
  const steps = [
    { target: ".book-title", content: "This displays the title of your book project." },
    { target: ".pages-progress", content: "Displays how many pages you’ve used so far. Each book slot includes up to 750 total pages." },
    { target: ".book-status", content: "Indicates your book’s current status." },
    { target: ".book-actions", content: "Use these buttons to add chapters, preview, or continue your book." },
  ];
        ...
        {books.map((b) => (
          ...
          >
            ...
            <div className="flex items-center justify-between text-[11px] text-gray-400 mb-2">
              ...
              <span>{new Date(b.created_at).toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" })}</span>
            </div>
            ...
            <div className="book-status text-xs font-semibold text-center mt-3">
              {b.status === "pending" ? (
                <div className="flex items-center justify-center gap-2 text-yellow italic">
                  <svg className="animate-spin h-4 w-4 text-yellow" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <span className="bg-red-500 text-white px-3 py-[3px] rounded-full">Failed</span>
              ) : b.pages >= 750 ? (
                <span className="bg-green text-black px-3 py-[3px] rounded-full">Completed</span>
              ) : b.pages > 0 ? (
                <span className="bg-blue text-white px-3 py-[3px] rounded-full">In Progress</span>
              ) : (
                <span className="bg-gray-600 text-white px-3 py-[3px] rounded-full">Idle</span>
              )}
            </div>
            ...
            <div className="text-[11px] text-gray-400 text-center mt-2">
              {b.created_at_prompt ? (
                <>
                  {new Date(b.created_at_prompt).toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" })}{" "}
                  {new Date(b.created_at_prompt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}
                </>
              ) : (
            ...
            <div className="book-actions flex flex-col items-stretch gap-2 mt-4 w-full">
              ...
              {b.pdf_url && (
                <>
                  ...
                  >
                    <div className="flex items-center justify-center gap-2">
                      <Eye size={16} className={`${b.pages >= 750 ? "text-green" : "text-white"}`} />
              ...
              {b.prompt && b.pages < 750 && (
                ...
                >
                  <Rocket size={16} />
                  <span>{b.pages >= 740 ? "Finish Book" : `Continue Story (Part ${(b.part_number || 1) + 1})`}</span>
                </button>
              )}
            </div>
          </motion.div>
        ...
        <OnboardingGuide
          ...
          onFinish={async () => {
            try {
              ...
              await api.post("https://cre8tlystudio.com/api/books/book-complete", { userId: user?.id }, {
